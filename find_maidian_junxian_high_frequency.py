...
# 计算均线
def add_moving_average(kline_df, window=10, GROUP_SIZE=1):
    kline_df[f'SMA_{junxian_window*GROUP_SIZE*6}seconds'] = kline_df['close'].rolling(window=window).mean()
    return kline_df
...
if __name__ == "__main__":
    ...
    return_test_timestamps = return_test_generate_timestamps(f"{start_date} 17:19:07", "2025-04-05 15:10:07", 10)
    ...
    ############################### 单次测试用 ###############################
        ...
        # if is_market_open(): # 线上实盘用
            ...
            # )
            df = read_all_csv_of_one_stock_some_days_after_date(
                stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=50,
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                junxian_kline_df = generate_kline_data_group_points(df, group_size=GROUP_SIZE*5)
                ...
                # duanjunxian_window = 100#78#100#125#625#125
                # changjunxian_window = 200#156#200#250#1250#250


                duanjunxian_window = 100  # 78#100#125#625#125
                changjunxian_window = 200  # 156#200#250#1250#250
                duanjunxian_window = 10  # 78#100#125#625#125
                changjunxian_window = 50  # 156#200#250#1250#250
                changchangjunxian_window = 300 #150 #100  # 156#200#250#1250#250
                changchangchangjunxian_window = 600  # 150 #100  # 156#200#250#1250#250


                #均线级别            6秒 12秒 30秒 60秒（1分钟） 120秒（2分钟） 180秒（3分钟）  300秒（5分钟）  600秒（10分钟）  1800秒（半小时）  3600秒（一小时）  7200秒（二小时）   14400秒（四小时）     21600秒（六小时，约等于一天，一天是6.5小时）
                #相当于最小时间粒度    1  2    5    10          20            30            50            100            300              600            1200              2400               3600
                junxian_window_sizes = [int(i/GROUP_SIZE) for  i in [1, 2, 5, 10, 20, 30, 50, 100, 300, 600, 1200, 2400, 3600] if i/GROUP_SIZE >= 1 and i%GROUP_SIZE == 0]
                # print(junxian_window_sizes)

                for junxian_window in junxian_window_sizes:
                    junxian_kline_df = add_moving_average(junxian_kline_df, window=junxian_window, GROUP_SIZE=GROUP_SIZE)

                # junxian_kline_df = add_moving_average(junxian_kline_df, window=duanjunxian_window)  # 计算短周期均线
                # junxian_kline_df = add_moving_average(junxian_kline_df, window=changjunxian_window)  # 计算长周期均线
                # junxian_kline_df = add_moving_average(junxian_kline_df, window=changchangjunxian_window)  # 计算长周期均线
                # junxian_kline_df = add_moving_average(junxian_kline_df, window=changchangchangjunxian_window)  # 计算长周期均线
                ...
                def plot_if_sanmai(sanmaitype="long", zhichengwei=0):
                    ...
                    # 为每个 window 生成颜色，使用 Viridis 渐变色，也可以换成 'Plasma', 'Cividis' 等
                    colors = sample_colorscale("Viridis", [i / (len(junxian_window_sizes) - 1) for i in
                                                           range(len(junxian_window_sizes))])

                    for jx, junxian_window in enumerate(junxian_window_sizes):
                        fig.add_trace(
                            go.Scatter(
                                x=junxian_kline_df['timestamp'],
                                y=junxian_kline_df[f'SMA_{junxian_window * GROUP_SIZE * 6}seconds'],
                                mode='lines',
                                name=f'{junxian_window * GROUP_SIZE * 6}seconds-period SMA',
                                line=dict(color=colors[jx], width=2)
                            ),
                            row=GROUP_SIZE_index * 3 + 2, col=1
                        )
                    ...
                    #     go.Scatter(
                    #         x=junxian_kline_df['timestamp'],
                    #         y=junxian_kline_df[f'SMA_{duanjunxian_window}'],
                    #         mode='lines',
                    ...
                    #     go.Scatter(
                    #         x=junxian_kline_df['timestamp'],
                    #         y=junxian_kline_df[f'SMA_{changjunxian_window}'],
                    #         mode='lines',
                    ...
                    #     go.Scatter(
                    #         x=junxian_kline_df['timestamp'],
                    #         y=junxian_kline_df[f'SMA_{changchangjunxian_window}'],
                    #         mode='lines',
                    #         name=f'{changchangjunxian_window}-period SMA',
                    #         line=dict(color='blue', width=2)
                    #     ),
                    #     row=GROUP_SIZE_index * 3 + 1, col=1
                    # )
                    ...
                    #     go.Scatter(
                    #         x=junxian_kline_df['timestamp'],
                    #         y=junxian_kline_df[f'SMA_{changchangchangjunxian_window}'],
                    #         mode='lines',
                    #         name=f'{changchangjunxian_window}-period SMA',
                    #         line=dict(color='blue', width=2)
                    #     ),
