...
if __name__ == "__main__":
    ...
    return_test_timestamps = return_test_generate_timestamps("2024-12-15 17:19:07", "2025-03-27 15:10:07", 10)
    return_test_timestamps = return_test_generate_timestamps("2024-12-15 17:19:07", "2025-12-27 15:10:07", 10)
    # time_until_list = [return_test_timestamps[i] for i in [-1]]
    # time_until_list = [return_test_timestamps[i] for i in [120, 240, 480]]
    time_until_list = [return_test_timestamps[i] for i in [3000]]
    ...
    ############################### 单次测试用 ###############################
        ...
        # if is_market_open(): # 线上实盘用
            ...
            # )
            df = read_all_csv_of_one_stock_some_days_after_date(
                stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=100,
                start_date="2024-12-15"
            )
            ######## 从最小级别向上递归 ######
            ############################### 全时间回测用 ###############################
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                # zhongjunxian_window = 250#1250#250
                # changjunxian_window = 500#1250#250
                duanjunxian_window = 5  # 625#125
                zhongjunxian_window = 60  # 1250#250
                changjunxian_window = 120 # 1250#250

                duanjunxian_window = 60  # 625#125
                zhongjunxian_window = 120  # 1250#250
                changjunxian_window = 240  # 1250#250
                kline_df = add_moving_average(kline_df, window=duanjunxian_window)  # 计算短周期均线
                kline_df = add_moving_average(kline_df, window=zhongjunxian_window)  # 计算中周期均线
                kline_df = add_moving_average(kline_df, window=changjunxian_window)  # 计算长周期均线

                changXjunxian_window = 480  # 1250#250
                kline_df = add_moving_average(kline_df, window=changXjunxian_window)  # 计算长周期均线




                # sma_columns = ['SMA_60', 'SMA_120', 'SMA_240', 'SMA_480']
                # sma_labels = ['a', 'b', 'c', 'd']
                sma_columns = ['SMA_60', 'SMA_120', 'SMA_240']
                sma_labels = ['a', 'b', 'c']
                def get_label_from_row(row):
                    values = [row[col] for col in sma_columns]
                    indexed = list(zip(sma_labels, values))
                    # 稳定排序，值一样时保持原始顺序
                    sorted_labels = [label for label, _ in sorted(indexed, key=lambda x: -x[1])]
                    return ''.join(sorted_labels)
                # 生成所有label序列
                label_series = kline_df.apply(get_label_from_row, axis=1)
                timestamps = kline_df['timestamp'].tolist()  # 假设timestamp列名是'timestamp'
                # 去除连续重复
                filtered_labels = []
                filtered_timestamps = []
                prev_label = None
                for label, ts in zip(label_series, timestamps):
                    if label != prev_label:
                ...
                # target_sequence = ['abcd', 'bacd', 'bcad', 'bcda', 'cbda', 'cdba', 'dcba']
                # seq_len = len(target_sequence)
                # match_indices = []
                # # 滑窗匹配
                # for i in range(len(filtered_labels) - seq_len + 1):
                #     window = filtered_labels[i:i + seq_len]
                #     if window == target_sequence:
                #         match_indices.append(i)
                # print(f"Total matches found: {len(match_indices)}")
                # print(f"Start indices: {match_indices}")
                ...
                # target_sequence = ['abcd', 'bacd', 'bcad', 'bcda', 'cbda', 'cdba', 'dcba']
                target_sequence = ['abc', 'bac', 'bca', 'cba']
                seq_len = 50
                label_len = len(filtered_labels)
                results = []
                target_timestamps = []
                for i in range(label_len - seq_len - len(target_sequence)):
                    sub_seq = filtered_labels[i:i + seq_len]
                    if sub_seq[-1] == 'abc':
                        next_seq = filtered_labels[i + seq_len - 1: i + seq_len + len(target_sequence) - 1]
                        if next_seq == target_sequence:
                            results.append(("short", sub_seq))
                            target_timestamps.append(
                                filtered_timestamps[i + seq_len - 1])  # 记录 target_sequence 的开始 timestamp
                        else:
                            results.append(("no_short", sub_seq))
                # 输出 summary
                print(f"Total sequences found: {len(results)}")
                print(f"Shorts: {sum(1 for r in results if r[0] == 'short')}")
                for r in results:
                    if r[0] == 'short':
                        print(r[1])
                print(f"No_shorts: {sum(1 for r in results if r[0] == 'no_short')}")
                ...
                # 拆分序列为 t1 ~ tseq_len
                sequence_df = pd.DataFrame([seq for _, seq in results], columns=[f"t{i + 1}" for i in range(seq_len)])
                sequence_df["label"] = [label for label, _ in results]
                output_path = "short_sequences.csv"
                # 如果 CSV 已存在，读取旧数据并拼接新数据
                if os.path.exists(output_path):
                    old_df = pd.read_csv(output_path)
                    combined_df = pd.concat([old_df, sequence_df], ignore_index=True)
                else:
                    combined_df = sequence_df
                # 保存为 CSV（覆盖写入）
                ...
                def plot_if_sanmai(sanmaitype="long", zhichengwei=0):
                    ...
                    fig.add_trace(
                        go.Scatter(
                            ...
                            y=kline_df[f'SMA_{zhongjunxian_window}'],
                            mode='lines',
                            name=f'{zhongjunxian_window}-period SMA',
                    ...
                    fig.add_trace(
                        go.Scatter(
                            x=kline_df['timestamp'],
                            y=kline_df[f'SMA_{changXjunxian_window}'],
                            mode='lines',
                            name=f'{changXjunxian_window}-period SMA',
                    ...
                    for timestamp in target_timestamps:
                        fig.add_trace(
                            go.Scatter(
                                ...
                                y=[kline_df['open'].min(), kline_df['open'].max()],  # 位置在线段下方，调整 -5 为适当的偏移值
                                mode='lines',
                                marker=dict(
                                    size=20,
                                    color="rgb(200, 0, 0)"
