...
from chanlun_utils import draw_zhongshu, CurrentPlay, save_new_segments_fix_to_checkpoint, merge_segments_fix_with_checkpoint, ensure_datetime, same_level_or_higher_level_before, check_zhongshu_kuozhang, zhongshu_kuozhang_merge_same_dir, greater_then_least_samelevel_tolerant, greater_then_least_samelevel_strict
import copy
...
if __name__ == "__main__":
    ...
    # return_test_timestamps = return_test_generate_timestamps("2024-12-16 17:19:07", "2025-06-03 18:10:07", 5)
    ...
    time_until_list = return_test_timestamps[80:-1]
    time_from_list = return_test_timestamps[0:-80]
    ...
    # ############################### 线上实盘用 ###############################
        ...
        # if is_market_open(): # 线上实盘用
            ...
            DAYS_LOOK_BASED_ON_GROUP_SIZE = {45: 20, 10: 10, 5: 15, 1: 15}
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                #     if fixed_pens[-1]["direction"] == "Up": #以找卖点为目标
                #         if fixed_pens[-1]["top_price"] < fixed_pens[-3]["top_price"]: #可以是二卖, fixed_pens[-3]["top_price"]可以是一卖
                #             if fixed_pens[-3]["top_price"] > fixed_pens[-5]["top_price"]: #fixed_pens[-3]["top_price"]是新高
                #                 if fixed_pens[-3]["top_price"] - fixed_pens[-3]["bottom_price"] < fixed_pens[-5]["top_price"] - fixed_pens[-5]["bottom_price"]: #fixed_pens[-3]相比fixed_pens[-5]背驰
                #                     if fixed_pens[-5]["bottom_price"] < fixed_pens[-3]["bottom_price"]: #fixed_pens[-5,-4,-3]是趋势
                #                         if (fixed_pens[-1]["bottom_price"] > fixed_pens[-5]["bottom_price"]) and (fixed_pens[-1]["top_price"] > fixed_pens[-3]["bottom_price"]): #给下跌留点空间,并且不操作三卖
                #
                #                             last_direction = fixed_pens[-1]["direction"]
                #                             current_play.update_yimai_by_direction(fixed_pens[-3], last_direction)
                ...
                #                             current_play.support_price = min(fixed_pens[-3]["bottom_price"], fixed_pens[-3]["bottom_price"] + (fixed_pens[-5]["top_price"] - fixed_pens[-5]["bottom_price"]))
                ...
                #             show_func=show_func
                #         )
                #     elif (min(fixed_pens[-1]["top_time"], fixed_pens[-1]["bottom_time"]) >= current_play.ermai_time):
                #         if fixed_pens[-1]["direction"] == "Down":
                #             if fixed_pens[-1]["top_price"] < current_play.ermai_price and fixed_pens[-1]["bottom_price"] < current_play.join_price: #已经在明确下跌，创出新低
                #                 if (fixed_pens[-1]["top_price"] - fixed_pens[-1]["bottom_price"]) < (fixed_pens[-3]["top_price"] - fixed_pens[-3]["bottom_price"]): #背驰
                #                     current_play.handle_pingcang(
                #                         df_row=df.iloc[-1],
                #                         info_save_mode=INFO_SAVE_TO_FILE_MODE,
                #                         print_return_info=PRINT_HUICE_RETURN_INFO,
                ...
                #                         show_func=show_func
                #                     )
                #         elif fixed_pens[-1]["direction"] == "Up": #前面一段没来得及读到
                #             if fixed_pens[-2]["top_price"] < current_play.ermai_price and fixed_pens[-2][
                #                 "bottom_price"] < current_play.join_price:  # 已经在明确下跌，创出新低
                #                 if (fixed_pens[-2]["top_price"] - fixed_pens[-2]["bottom_price"]) < (
                #                         fixed_pens[-4]["top_price"] - fixed_pens[-4]["bottom_price"]):  # 背驰
                #                     current_play.handle_pingcang(
                #                         df_row=df.iloc[-1],
                #                         info_save_mode=INFO_SAVE_TO_FILE_MODE,
                #                         print_return_info=PRINT_HUICE_RETURN_INFO,
                ...
                #                         show_func=show_func
                #                     )
                #         elif (min(fixed_pens[-3]["top_time"], fixed_pens[-3]["bottom_time"]) >= current_play.ermai_time): #二卖介入后走出三段段，看看有没有盘整
                ...
                if current_play.yimai_status == "inactivate" and (df.iloc[-1]['timestamp'].time() < datetime_time(18, 30)):
                    if fixed_pens[-2]["direction"] == "Up": #以找卖点为目标
                        if fixed_pens[-2]["top_price"] < fixed_pens[-4]["top_price"]: #可以是二卖, fixed_pens[-3]["top_price"]可以是一卖
                            if fixed_pens[-4]["top_price"] > fixed_pens[-6]["top_price"]: #fixed_pens[-3]["top_price"]是新高
                                if fixed_pens[-4]["top_price"] - fixed_pens[-4]["bottom_price"] < fixed_pens[-6]["top_price"] - fixed_pens[-6]["bottom_price"]: #fixed_pens[-4]相比fixed_pens[-6]背驰
                                    if fixed_pens[-6]["bottom_price"] < fixed_pens[-4]["bottom_price"]: #fixed_pens[-6,-5,-4]是趋势
                                        if (fixed_pens[-2]["bottom_price"] > fixed_pens[-6]["bottom_price"]) and (fixed_pens[-2]["top_price"] > fixed_pens[-4]["bottom_price"]): #给下跌留点空间,并且不操作三卖
                                            if fixed_pens[-1]["bottom_price"] > fixed_pens[-2]["bottom_price"] and fixed_pens[-2]["bottom_price"] < df.iloc[-1]['price'] < fixed_pens[-2]["top_price"]: #下拉段不破二卖形成段的低位， 当前价也还在二卖区间内
                                                print("二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖二卖")
                                                last_direction = fixed_pens[-2]["direction"]
                                                current_play.update_yimai_by_direction(fixed_pens[-4], last_direction)
                                                current_play.handle_ermai_confirmation(
                                                    segment=fixed_pens[-2],
                                                    df_row=df.iloc[-1],
                                                    info_save_mode=INFO_SAVE_TO_FILE_MODE,
                                                    print_return_info=PRINT_HUICE_RETURN_INFO,
                                                    show_fig=SHOW_FIG_WHEN_SELL_BUY_ACTION,
                                                ...
                                                current_play.join_price = df.iloc[-1]['price']
                                                current_play.support_price = min(fixed_pens[-4]["top_price"], fixed_pens[-4]["bottom_price"] + (fixed_pens[-6]["top_price"] - fixed_pens[-6]["bottom_price"]))
                elif current_play.yimai_status == "short":
                    if df.iloc[-1]['price'] >= current_play.support_price:
                        #击穿一卖就止损
                        current_play.handle_pingcang(
                        ...
                        #                 show_func=show_func
                        #             )
                        if fixed_pens[-1]["direction"] == "Up": #右侧交易，右侧止盈
                            if fixed_pens[-2]["top_price"] < current_play.ermai_price and fixed_pens[-2][
                                "bottom_price"] < current_play.join_price:  # 已经在明确下跌，创出新低
                                if (fixed_pens[-2]["top_price"] - fixed_pens[-2]["bottom_price"]) < (
                                        fixed_pens[-4]["top_price"] - fixed_pens[-4]["bottom_price"]):  # 背驰
                ...
                #             show_func=show_func,
                #             ersan_lianmai=True,
                #             zhongshu_before_ersanlianmai=segment_zhongshus_clean[-1]
                #         )
                #         current_play.join_price = df.iloc[-1]['price']
                #     else:
                #         current_play.reset()
