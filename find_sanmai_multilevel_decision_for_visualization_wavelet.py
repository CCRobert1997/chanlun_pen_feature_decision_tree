...
import time
# from machine_learning.gaussian_processing import gaussian_kernel_vectorization, GPR
from copy import deepcopy
import argparse
from chanlun_utils import generate_kline_data_group_points, handle_kline_inclusion_with_trend, \
    find_pens_from_kline, generate_feature_sequence, merge_pens_to_segments, \
    find_zhongshu_one_pen_form, clean_zhongshu_detailed, calculate_macd
...
if __name__ == "__main__":
    ...
    time_until_list = [return_test_timestamps[i] for i in [8]]
    ...
    if True:
        ...
        if True:  # 单次测试用 或者 全时间回测用
            ...
            ######## 从最小级别向上递归 ######
            df = read_all_csv_of_one_stock_some_days_after_date(
                stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=90,
                start_date="2024-12-15"
            )
            ######## 从最小级别向上递归 ######
            ############################### 全时间回测用 ###############################
            ...
            fig = make_subplots(rows=len(GROUP_SIZEs) * 3, cols=1, shared_xaxes=False, vertical_spacing=0.05,
                                subplot_titles=[
                                    'GROUP_SIZE' + str(GROUP_SIZEs[int(group_size_in_GROUP_SIZEs_index / 3)]) if (
                                                group_size_in_GROUP_SIZEs_index % 3 == 0 or group_size_in_GROUP_SIZEs_index % 3 == 1) else 'MACD'
                                    for group_size_in_GROUP_SIZEs_index in range(3 * len(GROUP_SIZEs))],
                                row_heights=[0.8 for group_i in range(3 * len(GROUP_SIZEs))])
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                # wavelet
                # wavelets = ['morl', 'mexh', 'cgau8', 'gaus8']
                signal = kline_df['open'].values
                # signal = signal - np.mean(signal)
                signal = (signal - np.mean(signal)) / np.std(signal)

                time_vals = np.arange(len(signal))
                widths = np.arange(1, 101)
                wavelet = 'gaus8'

                # 小波变换
                cwtmatr, _ = pywt.cwt(signal, widths, wavelet)
                central_freq = pywt.central_frequency(wavelet)
                freqs = central_freq / widths  # Hz，假设采样频率为1Hz

                # 重构（近似）
                # reconstructed = np.sum(np.real(cwtmatr), axis=0) / len(widths)
                reconstructed = np.sum(np.real(cwtmatr) / widths[:, np.newaxis], axis=0)
                # reconstructed /= np.max(np.abs(reconstructed))  # 可选归一化
                ...
                # Time3D, Freq3D = np.meshgrid(time_vals, freqs)
                # Amplitude = np.abs(cwtmatr)
                # 筛选低频部分（< 0.05 Hz）
                mask = freqs < 0.05
                freqs_low = freqs[mask]
                cwtmatr_low = cwtmatr[mask, :]
                Time3D, Freq3D = np.meshgrid(np.arange(len(signal)), freqs_low)
                Amplitude_low = np.abs(cwtmatr_low)
                # surf = ax1.plot_surface(Time3D, Freq3D, Amplitude, cmap=cm.jet, linewidth=0)
                surf = ax1.plot_surface(Time3D, Freq3D, Amplitude_low, cmap='jet', linewidth=0)
                ax1.set_title('3D CWT Surface (morl)')
                ax1.set_xlabel('Index')
                ax1.set_ylabel('Frequency (Hz)')
                ax1.set_zlabel('|CWT Coeff|')

                # 2. 原始 vs 重构
                ax2 = fig.add_subplot(2, 1, 2)
                ax2.plot(time_vals, signal, label='Original', marker='o', markersize=1)
                ax2.plot(time_vals, reconstructed, label='Reconstructed (approx)', linestyle='--', marker='x')
                ax2.set_title('Original vs Reconstructed Signal (morl)')
                ...
                # # print(df)
                # # df_train = df[df['timestamp'] <= "2024-12-17 17:46:07"]
                # df_train = df[df['timestamp'] <= "2024-12-17 17:10:07"]
                # df_train = df_train[df_train['timestamp'] >= "2024-12-16 19:40:07"]
                # print(df_train)
                # x_obs = df_train.index.to_numpy().reshape(-1, 1)  # 变成列向量
                # y_obs = df_train["price"].to_numpy()
                x_obs = np.array([pen["bottom_index"] if pen["direction"] == 'Up' else pen["top_index"] for pen in fixed_pens]).reshape(-1, 1)  # 变成列向量
                y_obs = np.array([pen["bottom_price"] if pen["direction"] == 'Up' else pen["top_price"] for pen in fixed_pens]).reshape(-1, 1)
                x_obs = x_obs[20:100]
                y_obs = y_obs[20:100]
                print(fixed_pens[100]["top_time"])
                print(fixed_pens[20]["top_time"])
                # print(x_obs)
                print(len(x_obs))
                # print(y_obs)
                print(len(y_obs))
                x_s = np.linspace(3500, 4500, 1001).reshape(-1, 1)  # 也是列向量
                print(gaussian_kernel_vectorization(x_obs, x_obs, l=500, sigma_f=10))
                gpr = GPR()
                gpr.fit(x_obs, y_obs)
                print(gpr.get_params())
                mu, cov = gpr.predict(x_s)
                y_mean = mu.ravel()
                uncertainty = 1.96 * np.sqrt(np.diag(cov))
                # print(y_mean)
                # # 设置核函数（RBF + 噪声）
                # kernel = RBF(length_scale=10.0) + WhiteKernel(noise_level=5.0)
                # # 建立模型
                # gp = GaussianProcessRegressor(kernel=kernel, random_state=0)
                # # 拟合数据
                # gp.fit(x_obs, y_obs)
                # # 进行预测
                # y_mean, y_std = gp.predict(x_s, return_std=True)
                # 找出 x_s 对应的 timestamp
                timestamps_s = df.loc[x_s.ravel(), 'timestamp'].reset_index(drop=True)
                # 构建预测结果 DataFrame
                ...
                if segments:
                    if segments_fix:
                        ...
                        segment_zhongshus_clean, segment_zhuanzhes_clean, sanmai_info = clean_zhongshu_detailed(
                            segment_zhongshus, segments_fix)
