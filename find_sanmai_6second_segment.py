                    ...
                    if len(segments_fix) - 1 >= segment_zhongshus[-1]['core_pens_index'][-1] + 1:
                        ...
                        if (segments_fix[-1]['bottom_price'] > segment_zhongshus[-1]['ZG']) and (
                            ...
                            if len(segment_zhongshus) >= 3:
                                # if df.iloc[-1]['price'] > segment_zhongshus[-1]['ZG']:
                                if not san_buy_appeared:
                                    # 当前价格不能高于前一个线段的高点,也就是三买形成后不要走太远
                                    maidian_juli = "买点出现不远，价值高" if (df.iloc[-1]['price'] < segments_fix[-2]['top_price']) else "买点已经有点距离，价值略降低"
                                    if (segment_zhongshus[-1]['ZG'] < segment_zhongshus[-2]['ZD']) and (
                                        ...
                                        if (len(segment_zhongshus) >= 4) and (segment_zhongshus[-3]['ZG'] < segment_zhongshus[-4]['ZD']):
                                            ...
                                            write_to_sanmai_info_file(data_sanmai)
                                            if PRINT_PROCESS_INFO:
                                                print("六秒级线段弱三买！！！！！！六秒级线段弱三买！！！！！！！！六秒级线段弱三买！！！！！！！！！！！！！")
                                        # print("疑似三买！！！！！！三买！！！！！！！！三买！！！！！！！！！！！！！")
                    ...
                    if len(segments_fix) - 1 >= segment_zhongshus[-1]['core_pens_index'][-1] + 1:
                        ...
                        if (segments_fix[-1]['top_price'] < segment_zhongshus[-1]['ZD']) and (
                            ...
                            if len(segment_zhongshus) >= 3:
                                ...
                                # if df.iloc[-1]['price'] < segment_zhongshus[-1]['ZD']:
                                if not san_sell_appeared:
                                    # 当前价格不能高于前一个线段的高点,也就是三买形成后不要走太远
                                    maidian_juli = "卖点出现不远，价值高" if (df.iloc[-1]['price'] > segments_fix[-2]['bottom_price']) else "卖点已经有点距离，价值略降低"
                                    if (segment_zhongshus[-1]['ZD'] > segment_zhongshus[-2]['ZG']) and (
                                        ...
                                        if (len(segment_zhongshus) >= 4) and (segment_zhongshus[-3]['ZD'] > segment_zhongshus[-4]['ZG']):
                                            ...
                                            if PRINT_PROCESS_INFO:
                                                print("六秒级线段弱三卖！！！！！！六秒级线段弱三卖！！！！！！！！六秒级线段弱三卖！！！！！！！！！！！！！")
                                            write_to_sanmai_info_file(data_sanmai)
                                        # print("疑似三卖！！！！！！三卖！！！！！！！！三卖！！！！！！！！！！！！！")
