...
if __name__ == "__main__":
    ...
    if True:
        ...
        # if is_market_open():
            ...
            # df = df[df['timestamp'] <= "2025-01-17 19:13:07"]
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                GROUP_SIZE_FOR_MACD = GROUP_SIZE * 125
                ...
                #                             if (len(segment_zhongshus) >= 4) and (segment_zhongshus[-3]['ZG'] < segment_zhongshus[-4]['ZD']):
                #                                 data_sanmai = {
                #                                     "sanmai_state": "线段高价值三买出现",
                #                                     "info": f"前方四段中枢，趋势转折概率极大，在{chengben_sanbuy}做多, 支撑在{qian_top_before_san_buy}, 在{huilajianyan_time_for_san_buy}时操作"
                #                                 }
                #                                 # write_to_sanmai_info_file(data_sanmai)
                #                                 if PRINT_PROCESS_INFO:
                ...
                #                                     "info": f"前方三段中枢，有可能只是盘整后的转折，可以考虑介入，在{chengben_sanbuy}做多, 支撑在{qian_top_before_san_buy}, 在{huilajianyan_time_for_san_buy}时操作"
                #                                 }
                #                                 # write_to_sanmai_info_file(data_sanmai)
                #                                 if PRINT_PROCESS_INFO:
                #                                     print("长线弱三买！！！！！！长线弱三买！！！！！！！！长线弱三买！！！！！！！！！！！！！")
                #                             # print("疑似三买！！！！！！三买！！！！！！！！三买！！！！！！！！！！！！！")
                #                             # print("评估一下前面有没有一买！是否前面是低点！")
                #                             # print("注意要非常小心止损，回到三买前的中枢要止损，回跌过大，比如1%要止损")
                #                             plot_if_sanmai('long', str(segment_zhongshus[-1]['ZG']) + "_buyprice_" + str(df.iloc[-1]['price']))
                #                 elif (segment_zhongshus[-1]['ZG'] < segment_zhongshus[-2]['ZD']):
                #                     san_buy_appeared = True
                #                     san_sell_appeared = False
                #                     data_sanmai = {
                #                         "sanmai_state": "类似三买，疑似线段三卖后盘整或趋势结束",
                #                         "info": f"有仓平仓，无仓不建仓，新中枢低点{segment_zhongshus[-1]['ZD']}高于原中枢{segment_zhongshus[-2]['ZD']}"
                #                     }
                #                     # write_to_sanmai_info_file(data_sanmai)
                #                     if PRINT_PROCESS_INFO:
                #                         print("不值得交易的三买！")
                #                     plot_if_sanmai('long_no_trade', str(segment_zhongshus[-1]['ZG']) + "_buyprice_" + str(
                #                         df.iloc[-1]['price']))
                #
                #
                # # 寻找三卖
                ...
                #                             if (len(segment_zhongshus) >= 4) and (segment_zhongshus[-3]['ZD'] > segment_zhongshus[-4]['ZG']):
                #                                 data_sanmai = {
                #                                     "sanmai_state": "线段高价值三卖出现",
                #                                     "info": f"前方四段中枢，趋势转折概率极大，在{chengben_sansell}做空, 支撑在{qian_bottom_before_san_sell}, 在{huilajianyan_time_for_san_sell}时操作"
                #                                 }
                #                                 if PRINT_PROCESS_INFO:
                ...
                #                                     "info": f"前方三段中枢，有可能只是盘整后的转折，可以考虑介入，在{chengben_sansell}做空, 支撑在{qian_bottom_before_san_sell}, 在{huilajianyan_time_for_san_sell}时操作"
                #                                 }
                #                                 if PRINT_PROCESS_INFO:
                #                                     print("长线弱三卖！！！！！！长线弱三卖！！！！！！！！长线弱三卖！！！！！！！！！！！！！")
                #                                 # write_to_sanmai_info_file(data_sanmai)
                #                             # print("疑似三卖！！！！！！三卖！！！！！！！！三卖！！！！！！！！！！！！！")
                #                             # print("评估一下前面有没有一卖！是否前面是高点！")
                #                             # print("注意要非常小心止损，回到三卖前的中枢要止损，回涨过大，比如1%要止损")
                #                             plot_if_sanmai('short', str(segment_zhongshus[-1]['ZD']) + "_shortprice_" + str(df.iloc[-1]['price']))
                #
                ...
                #                         "info": f"有仓平仓，无仓不建仓，新中枢高点{segment_zhongshus[-1]['ZG']}低于原中枢{segment_zhongshus[-2]['ZG']}"
                #                     }
                #                     # write_to_sanmai_info_file(data_sanmai)
                #                     if PRINT_PROCESS_INFO:
                #                         print("不值得交易的三卖！")
                #                     plot_if_sanmai('short_no_trade', str(segment_zhongshus[-1]['ZD']) + "_shortprice_" + str(
                #                         df.iloc[-1]['price']))
                # 寻找三买，无缺口的情况
                if segments_fix[-1]['direction'] == 'Down':  # 即离开中枢后的拉回段
                    # 给线段0开始编号，最后一个线段的编号就是 len(segments_fix)-1
                            ...
                            if len(segment_zhongshus) >= 3:
                                if not san_buy_appeared:
                                    ...
                                    if (segment_zhongshus[-1]['ZG'] < segment_zhongshus[-2]['ZD']) and (
                                        ...
                                        san_sell_appeared = False
                                        data_sanmai = {
                                            "sanmai_state": "六秒级线段类似三买，疑似线段三卖后盘整或趋势结束",
                                ...
                                if not san_sell_appeared:
                                    ...
                                    if (segment_zhongshus[-1]['ZD'] > segment_zhongshus[-2]['ZG']) and (
                                        ...
                                        if (len(segment_zhongshus) >= 4) and (
                                            ...
                                            if PRINT_PROCESS_INFO:
                                                print("六秒级线段三卖！！！！！！六秒级线段三卖！！！！！！！！六秒级线段三卖！！！！！！！！！！！！！")
                                            # write_to_sanmai_info_file(data_sanmai)
                                        else:
                                            data_sanmai = {
                                                "sanmai_state": "六秒级线段低价值三卖出现",
                                        ...
                                        san_sell_appeared = True
                                        data_sanmai = {
                                            "sanmai_state": "六秒级线段类似三卖，疑似线段三买后盘整或趋势结束",
        ...
        # time.sleep(180)  # Wait for 180 seconds (3 minutes) before the next attempt
