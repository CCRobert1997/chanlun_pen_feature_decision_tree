...
from chanlun_utils import draw_zhongshu, CurrentPlay, save_new_segments_fix_to_checkpoint, merge_segments_fix_with_checkpoint, ensure_datetime, same_level_or_higher_level_before, check_zhongshu_kuozhang, zhongshu_kuozhang_merge_same_dir
import copy
...
if __name__ == "__main__":
    ...
    # time_until_list = return_test_timestamps[1080+280+224:-1]
    # time_from_list = return_test_timestamps[0+280+224:-1081]
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                if segments:
                    if segments_fix:
                        ...
                        save_new_segments_fix_to_checkpoint(segments_fix, segment_zhongshus_clean, STOCK_NAME_AND_MARKET)
                ...
                if segments_fix:
                    history_long_time_segments = merge_segments_fix_with_checkpoint(segments_fix, STOCK_NAME_AND_MARKET)
                    ...
                    #画更大级别线
                    high_level_segments, high_level_type_three_buy_sell = merge_pens_to_segments_based_on_pen_zhongshu(history_long_time_segments,
                                                                                                     history_long_time_segment_zhongshus_clean)
                ...
                # 处理最后一个方向上的中枢扩张
                # 处理最后一个方向上的中枢扩张
                current_dir_for_check_zhongshu_kuozhang = history_long_time_segment_zhongshus_clean[-1][
                    "direction"]
                cut_index = -1
                for i in range(len(history_long_time_segment_zhongshus_clean) - 1):
                    if history_long_time_segment_zhongshus_clean[-i - 2][
                        "direction"] == current_dir_for_check_zhongshu_kuozhang:
                        cut_index = -i - 2
                    else:
                        break
                history_long_time_segment_zhongshus_clean_SEC_1 = history_long_time_segment_zhongshus_clean[
                                                                  :cut_index]
                history_long_time_segment_zhongshus_clean_SEC_2 = history_long_time_segment_zhongshus_clean[
                                                                  cut_index:]
                history_long_time_segment_zhongshus_clean_SEC_2_rebuild = [
                    history_long_time_segment_zhongshus_clean_SEC_2[0]]
                for i in range(len(history_long_time_segment_zhongshus_clean_SEC_2) - 1):
                    if check_zhongshu_kuozhang(history_long_time_segment_zhongshus_clean_SEC_2_rebuild[-1],
                                               history_long_time_segment_zhongshus_clean_SEC_2[i + 1]):
                        rebuild_zhongshu = zhongshu_kuozhang_merge_same_dir(
                            history_long_time_segment_zhongshus_clean_SEC_2_rebuild[-1],
                            history_long_time_segment_zhongshus_clean_SEC_2[i + 1], history_long_time_segments)
                        # if rebuild_zhongshu["core_pens_index"][-1] == len(history_long_time_segments) - 1:
                        #     #这里最后一段可能被算入扩张的中枢，要再截出来
                        #     rebuild_zhongshu["core_pens_index"] = rebuild_zhongshu["core_pens_index"][:-1]
                        history_long_time_segment_zhongshus_clean_SEC_2_rebuild[-1] = rebuild_zhongshu
                    else:
                        history_long_time_segment_zhongshus_clean_SEC_2_rebuild.append(
                            history_long_time_segment_zhongshus_clean_SEC_2[i + 1])
                history_long_time_segment_zhongshus_clean = history_long_time_segment_zhongshus_clean_SEC_1 + history_long_time_segment_zhongshus_clean_SEC_2_rebuild

                # if history_long_time_segment_zhongshus_clean[-1]["core_pens_index"][-1] == len(history_long_time_segments) - 1:
                #     #再处理一下同向三mai，没有扩张的情况
                #     history_long_time_segment_zhongshus_clean = history_long_time_segment_zhongshus_clean[:-1]
                #     history_long_time_segment_zhongshus_clean[-1]["core_pens_index"].append(history_long_time_segment_zhongshus_clean[-1]["core_pens_index"][-1]+1)
                #
                ...
                if current_play.yimai_status in {"long", "short"}:
                    ...
                    current_play.zhiying_or_keep_operation(
                        zhongshus=history_long_time_segment_zhongshus_clean,
                        segments_fix=history_long_time_segments,
                        df_row=df.iloc[-1],
                        info_save_mode=INFO_SAVE_TO_FILE_MODE,
                        print_return_info=PRINT_HUICE_RETURN_INFO,
                        show_fig=SHOW_FIG_WHEN_SELL_BUY_ACTION,
                        ...
                        plot_complete_func=plot_complete,
                        show_func=show_func,
                        sanmai_info=history_long_time_sanmai_info,
                        new_zhongshu_clean=history_long_time_new_zhongshu_clean
                    )
                ...
                if current_play.yimai_status in {"candidate_long", "candidate_short"} and (
                        not bool(history_long_time_new_zhongshu_clean) and (not same_level_or_higher_level_before(history_long_time_segment_zhongshus_clean, history_long_time_segments))):
                    #中枢升级的判定，如果有一mai，清掉，不该再找二mai
                    print("中枢延伸，升级")
                    current_play.reset()
                #if history_long_time_segment_zhongshus_clean """ now we have two same direction zhongshu""" and """ I am 空仓 current_play.operation_direction == "" """:
                #因为构成三买的中枢被拿掉了，所以用len(history_long_time_segment_zhongshus_clean) + bool(history_long_time_sanmai_info)补回
                # if (not (current_play.yimai_status in {"long", "short"})) and (len(history_long_time_segment_zhongshus_clean) + bool(history_long_time_sanmai_info)) >= 3 and ((history_long_time_segment_zhongshus_clean[-1]["direction"] == history_long_time_segment_zhongshus_clean[-2]["direction"]) or (history_long_time_new_zhongshu_clean and (history_long_time_segment_zhongshus_clean[-1]["direction"] == history_long_time_new_zhongshu_clean["direction"]))):
                # elif (not (current_play.yimai_status in {"long", "short"})) and ((len(history_long_time_segment_zhongshus_clean) + bool(history_long_time_sanmai_info)) >= 3) and ((not bool(history_long_time_new_zhongshu_clean)) or (history_long_time_new_zhongshu_clean and (history_long_time_segment_zhongshus_clean[-1]["direction"] != history_long_time_new_zhongshu_clean["direction"]))):
                elif (not (current_play.yimai_status in {"long", "short"}))  and ((not bool(history_long_time_new_zhongshu_clean)) or (history_long_time_new_zhongshu_clean and (history_long_time_segment_zhongshus_clean[-1]["direction"] != history_long_time_new_zhongshu_clean["direction"]))):
                    last_direction = history_long_time_segment_zhongshus_clean[-1]["direction"]

                    beichi_check_segment_new = history_long_time_segments[history_long_time_segment_zhongshus_clean[-1]["core_pens_index"][-1] + 1]
                    beichi_check_segment_old = history_long_time_segments[history_long_time_segment_zhongshus_clean[-1]["core_pens_index"][0] - 1]
                    if beichi_check_segment_new["direction"] != beichi_check_segment_old["direction"]:
                        beichi_check_segment_new = history_long_time_segments[history_long_time_segment_zhongshus_clean[-1]["core_pens_index"][-1]]

                    if current_play.yimai_status == "inactivate":
                        if last_direction == history_long_time_segments[-1]["direction"]:
                            current_play.check_beichi_and_update_yimai(history_long_time_segment_zhongshus_clean[-1], beichi_check_segment_new, beichi_check_segment_old, last_direction)
                        ...
                        if last_direction == history_long_time_segments[-1]["direction"]:
                            # if beichi:
                            detect_zhongshu_kuozhang = current_play.detect_zhongshu_expansion(beichi_check_segment_new, history_long_time_segment_zhongshus_clean[-2])
                            if detect_zhongshu_kuozhang:
                                ...
                                if history_long_time_sanmai_info == "":
                                    ...
                                    if max(history_long_time_segments[-1]["top_time"],
                                           history_long_time_segments[-1]["bottom_time"]) > last_operation_confirm_time:
                                        print("二三连mai确认mai点")
                                        ...
                                        last_operation_confirm_time = max(history_long_time_segments[-1]["top_time"], history_long_time_segments[-1]["bottom_time"])
                            else: # 如果不扩展
                                #
                                print(f"一买后确认没有扩展，找二买最后一步 {df.iloc[-1]['timestamp']}")
                                if current_play.check_ermai_by_compare_to_yimai(history_long_time_segments[-1]):
                                    if max(history_long_time_segments[-1]["top_time"], history_long_time_segments[-1]["bottom_time"]) > last_operation_confirm_time:
                                    ...
                                    current_play.check_beichi_and_update_yimai(history_long_time_segment_zhongshus_clean[-1],
