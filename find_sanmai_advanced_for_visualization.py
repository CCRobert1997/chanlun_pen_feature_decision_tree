...
if __name__ == "__main__":
    ...
    if True:
        ...
        # if is_market_open():
            ...
            if GROUP_SIZEs[0] == 1:
                df = read_all_csv_of_one_stock_some_days(stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=11)
                # df = read_all_csv_of_one_stock_ten_days(stock_name_and_market=STOCK_NAME_AND_MARKET)
            else:
                #####################读取某个股的最近50天数据###########################
                df = read_all_csv_of_one_stock_some_days(stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=50)
                # df = read_all_csv_of_one_stock_some_days(stock_name_and_market=STOCK_NAME_AND_MARKET, days_n=11)
                # df = read_all_csv_of_one_stock_fifty_days(stock_name_and_market=STOCK_NAME_AND_MARKET)
            """
            50min: 500
            37p5min: 375
            25min: 250
            ...
            # 创建子图布局
            fig = make_subplots(rows=len(GROUP_SIZEs)*3+1, cols=1, shared_xaxes=True, vertical_spacing=0.05,
                                subplot_titles=['basetime']+['GROUP_SIZE' + str(GROUP_SIZEs[int(group_size_in_GROUP_SIZEs_index/3)]) if (group_size_in_GROUP_SIZEs_index%3==0 or group_size_in_GROUP_SIZEs_index%3==1) else 'MACD' for group_size_in_GROUP_SIZEs_index in range(3*len(GROUP_SIZEs))], row_heights=[0.7 for group_i in range(3*len(GROUP_SIZEs) + 1)])
            ...
            for GROUP_SIZE_index, GROUP_SIZE in enumerate(GROUP_SIZEs):
                ...
                duanjunxian_window = 625#125
                changjunxian_window = 1250#250
                ...
                def plot_if_sanmai(sanmaitype="long", zhichengwei=0):
                    ...
                    draw_zhongshu(fig, pen_zhongshus, row=GROUP_SIZE_index * 3 + 2, col=1, level="pen")
                    draw_zhongshu(fig, segment_zhongshus, row=GROUP_SIZE_index * 3 + 2, col=1, level="segment")
                    ...
                    ), row=GROUP_SIZE_index * 3 + 4, col=1)
                    ...
                    # Add Zhongshu Rectangles to the chart
                    draw_zhongshu(fig, pen_zhongshus_clean, row=GROUP_SIZE_index * 3 + 3, col=1, level="pen")
                    draw_zhongshu(fig, segment_zhongshus_clean, row=GROUP_SIZE_index * 3 + 3, col=1, level="segment")
                    # draw_zhongshu(fig, segment_of_segment_zhongshus, row=GROUP_SIZE_index*2 + 2, col=1, level="segment_of_segment")
                    ...
                    fig.update_layout(
                        ...
                        height=500 * (3 * len(GROUP_SIZEs) + 1),
                    )

                    for i in range(len(GROUP_SIZEs) * 3 + 1):
                    ...
                    fig.write_image(file_name_record_if_sanmai, width=3000, height=2000)
                ...
                if san_buy_appeared and (df.iloc[-1]['price'] < qian_top_before_san_buy) and (segment_zhongshus[-1]['start_time'] < huilajianyan_time_for_san_buy):
                    san_buy_appeared = False
                ...
                if san_sell_appeared and (df.iloc[-1]['price'] > qian_bottom_before_san_sell) and (segment_zhongshus[-1]['start_time'] < huilajianyan_time_for_san_sell):
                    san_sell_appeared = False
                ...
                if san_buy_appeared and (segment_zhongshus[-1]['end_time'] > huilajianyan_time_for_san_buy) and (segment_zhongshus[-1]['ZG'] < segment_zhongshus[-2]['ZG']):
                    # 条件1 是三买后，条件2是三买后已经出现中枢，条件三是新中枢说明3买后的涨势已经结束
                    if huilajianyan_time_for_san_buy >= segment_zhongshus[-2]['start_time']:
                        #这个条件如果不满足，就不是盘整了，是趋势了
                        san_buy_appeared = False
                        san_sell_appeared = True

                        data_sanmai = {
                            "sanmai_state": "线段三买后明确盘整",
                            "info": f"新中枢高点{segment_zhongshus[-1]['ZG']}低于原中枢{segment_zhongshus[-2]['ZG']}"
                        }
                        # write_to_sanmai_info_file(data_sanmai)
                        plot_if_sanmai('stop_long_panzheng', "")

                if san_sell_appeared and (segment_zhongshus[-1]['end_time'] > huilajianyan_time_for_san_sell) and (segment_zhongshus[-1]['ZD'] > segment_zhongshus[-2]['ZD']):
                    if huilajianyan_time_for_san_sell >= segment_zhongshus[-2]['start_time']:
                        san_sell_appeared = False
                        san_buy_appeared = True
                        data_sanmai = {
                            "sanmai_state": "线段三卖后明确盘整",
                            "info": f"新中枢低点{segment_zhongshus[-1]['ZD']}高于原中枢{segment_zhongshus[-2]['ZD']}"
                        }
                        # write_to_sanmai_info_file(data_sanmai)
                ...
                if sanmai_info == "long":
                    # 实时价依旧高于前中枢
                    if (df.iloc[-1]['price'] > segment_zhongshus_clean[-2]['ZG']):
                        if len(segment_zhongshus_clean) >= 3:
                            # 仅交易新走势第一个中枢
                            if segment_zhongshus_clean[-1]['direction'] != segment_zhongshus_clean[-2]['direction']:
                                # 仅交易趋势背驰后的三买
                                if segment_zhongshus_clean[-2]['direction'] == segment_zhongshus_clean[-3]['direction']:
                                    # 三买之后不要走出太远
                                    # if (df.iloc[-1]['price'] < segments_fix[-2]['top_price']):
                                    maidian_juli = "买点出现不远，价值高" if (
                                            df.iloc[-1]['price'] < segments_fix[-2][
                                        'top_price']) else "买点已经有点距离，风险提升"
                                    san_buy_appeared = True
                                    qian_top_before_san_buy = segment_zhongshus[-2]['ZG']
                                    chengben_sanbuy = df.iloc[-1]['price']
                                    huilajianyan_time_for_san_buy = segments_fix[-1]["bottom_time"]
                                    san_sell_appeared = False
                                    data_sanmai = {
                                        "sanmai_state": "线段高价值三买出现",
                                        "info": f"{maidian_juli}，在{chengben_sanbuy}做多, 支撑在{qian_top_before_san_buy}, 在{huilajianyan_time_for_san_buy}时操作"
                                    }
                                    if PRINT_PROCESS_INFO:
                                        print("长线三买！！！！！！长线三买！！！！！！！！长线三买！！！！！！！！！！！！！")
                                        print(data_sanmai)
                                    # write_to_sanmai_info_file(data_sanmai)
                elif sanmai_info == "short":
                    # 实时价依旧低于前中枢
                    if (df.iloc[-1]['price'] < segment_zhongshus_clean[-2]['ZD']):
                        if len(segment_zhongshus_clean) >= 3:
                            # 仅交易新走势第一个中枢
                            if segment_zhongshus_clean[-1]['direction'] != segment_zhongshus_clean[-2]['direction']:
                                # 仅交易趋势背驰后的三卖
                                if segment_zhongshus_clean[-2]['direction'] == segment_zhongshus_clean[-3]['direction']:
                                    # 三卖之后不要走出太远
                                    # if (df.iloc[-1]['price'] > segments_fix[-2]['bottom_price']):
                                    maidian_juli = "卖点出现不远，价值高" if (
                                            df.iloc[-1]['price'] > segments_fix[-2][
                                        'bottom_price']) else "卖点已经有点距离，风险提升"
                                    san_sell_appeared = True
                                    qian_bottom_before_san_sell = segment_zhongshus[-2]['ZD']
                                    chengben_sansell= df.iloc[-1]['price']
                                    huilajianyan_time_for_san_sell = segments_fix[-1]["top_time"]
                                    san_buy_appeared = False
                                    data_sanmai = {
                                        "sanmai_state": "线段高价值三卖出现",
                                        "info": f"{maidian_juli}，在{chengben_sansell}做多, 支撑在{qian_bottom_before_san_sell}, 在{huilajianyan_time_for_san_sell}时操作"
                                    }
                                    if PRINT_PROCESS_INFO:
                                        print("长线三卖！！！！！！长线三卖！！！！！！！！长线三卖！！！！！！！！！！！！！")
                                        print(data_sanmai)
                                    # write_to_sanmai_info_file(data_sanmai)
                ...
                # # 寻找三买，无缺口的情况
                # if segments_fix[-1]['direction'] == 'Down':  # 即离开中枢后的拉回段
                #     # 给线段0开始编号，最后一个线段的编号就是 len(segments_fix)-1
                #     # 最后一个线段作为最后一个中枢的离开段 segment_zhongshus[-1]['core_pens_index'][-1] + 2 或 segment_zhongshus[-1]['core_pens_index'][-1] + 1
                #     # 因为离开段可能被画进中枢
                #     if len(segments_fix) - 1 >= segment_zhongshus[-1]['core_pens_index'][-1] + 1:
                #         # 检查中枢突破，最后一个线段最低价要高于中枢最高价，当前价也要高于中枢最高价
                #         if (segments_fix[-1]['bottom_price'] > segment_zhongshus[-1]['ZG']) and (
                ...
                #                         # 如果三买已经出现，就不重复标记了
                #                         san_buy_appeared = True
                #                         qian_top_before_san_buy = segment_zhongshus[-1]['ZG']
                #                         chengben_sanbuy = df.iloc[-1]['price']
                #                         huilajianyan_time_for_san_buy = segments_fix[-1]["bottom_time"]
                #                         san_sell_appeared = False
                #                         # print("三买出现", f"在{chengben_sanbuy}做多", f"支撑在{qian_top_before_san_buy}", f"在{huilajianyan_time_for_san_buy}时操作")
                #                         if (len(segment_zhongshus) >= 4) and (
                #                                 segment_zhongshus[-3]['ZG'] < segment_zhongshus[-4]['ZD']):
                #                             data_sanmai = {
                #                                 "sanmai_state": "线段高价值三买出现",
                ...
                #                         else:
                #                             data_sanmai = {
                #                                 "sanmai_state": "线段低价值三买出现",
                #                                 "info": f"{maidian_juli}, 前方三段中枢，有可能只是盘整后的转折，可以考虑介入，在{chengben_sanbuy}做多, 支撑在{qian_top_before_san_buy}, 在{huilajianyan_time_for_san_buy}时操作"
                #                             }
                #                             # write_to_sanmai_info_file(data_sanmai)
                #                             if PRINT_PROCESS_INFO:
                #                                 print(
                ...
                #                         plot_if_sanmai('long', str(
                #                             segment_zhongshus[-1]['ZG']) + "_buyprice_" + str(
                #                             df.iloc[-1]['price']))
                #                     elif (segment_zhongshus[-1]['ZG'] < segment_zhongshus[-2]['ZD']):
                #                         san_buy_appeared = True
                #                         san_sell_appeared = False
                #                         data_sanmai = {
                #                             "sanmai_state": "类似三买，疑似线段三卖后盘整或趋势结束",
                #                             "info": f"有仓平仓，无仓不建仓，{maidian_juli}, 新中枢低点{segment_zhongshus[-1]['ZD']}高于原中枢{segment_zhongshus[-2]['ZD']}"
                #                         }
                #                         # write_to_sanmai_info_file(data_sanmai)
                #                         if PRINT_PROCESS_INFO:
                #                             print("不值得交易的三买！")
                #                         plot_if_sanmai('long_no_trade', str(
                #                             segment_zhongshus[-1]['ZG']) + "_buyprice_" + str(
                #                             df.iloc[-1]['price']))
                #
                # # 寻找三卖
                # if segments_fix[-1]['direction'] == 'Up':  # 即离开中枢后的拉回段
                #     # 给线段0开始编号，最后一个线段的编号就是 len(segments_fix)-1
                #     # 最后一个线段作为最后一个中枢的离开段 segment_zhongshus[-1]['core_pens_index'][-1] + 2 或 segment_zhongshus[-1]['core_pens_index'][-1] + 1
                #     # 因为离开段可能被画进中枢
                #     if len(segments_fix) - 1 >= segment_zhongshus[-1]['core_pens_index'][-1] + 1:
                #         # 检查中枢突破，最后一个线段最低价要低于中枢最低价，当前价也要低于中枢最低价
                #         if (segments_fix[-1]['top_price'] < segment_zhongshus[-1]['ZD']) and (
                ...
                #                     maidian_juli = "卖点出现不远，价值高" if (
                #                                 df.iloc[-1]['price'] > segments_fix[-2][
                #                             'bottom_price']) else "卖点已经有点距离，价值略降低"
                ...
                #                         else:
                #                             data_sanmai = {
                #                                 "sanmai_state": "线段低价值三卖出现",
                #                                 "info": f"{maidian_juli}, 前方三段中枢，有可能只是盘整后的转折，可以考虑介入，在{chengben_sansell}做空, 支撑在{qian_bottom_before_san_sell}, 在{huilajianyan_time_for_san_sell}时操作"
                #                             }
                #                             if PRINT_PROCESS_INFO:
                #                                 print(
                #                                     "长线弱三卖！！！！！！长线弱三卖！！！！！！！！长线弱三卖！！！！！！！！！！！！！")
                ...
                #                         plot_if_sanmai('short', str(
                #                             segment_zhongshus[-1]['ZD']) + "_shortprice_" + str(
                #                             df.iloc[-1]['price']))
                #
                #                     elif (segment_zhongshus[-1]['ZD'] > segment_zhongshus[-2]['ZG']):
                #                         san_buy_appeared = False
                #                         san_sell_appeared = True
                #                         data_sanmai = {
                #                             "sanmai_state": "类似三卖，疑似线段三买后盘整或趋势结束",
                #                             "info": f"有仓平仓，无仓不建仓，{maidian_juli}, 新中枢高点{segment_zhongshus[-1]['ZG']}低于原中枢{segment_zhongshus[-2]['ZG']}"
                #                         }
                #                         # write_to_sanmai_info_file(data_sanmai)
                #                         if PRINT_PROCESS_INFO:
                #                             print("不值得交易的三卖！")
                #                         plot_if_sanmai('short_no_trade', str(
                #                             segment_zhongshus[-1]['ZD']) + "_shortprice_" + str(
                #                             df.iloc[-1]['price']))
